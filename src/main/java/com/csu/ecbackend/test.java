...
//    private static final String[] words ={"华为","宝马","斗破苍穹","面膜","乐视","周杰伦","月饼","长沙","苹果","钢琴","杨洋","淘宝","韩国","教师","微信"};
...
////
////            read readData_clean=new read(filrPath+(i+1),cleanPath+(i+1)+".txt","clean");
...
//            fre.getFrequency();*/
//
//            read readData_contain=new read(new Path().getYuanPathClean(),new Path().getYuanPathContain()+words[i]+".txt","contain");
//            //System.out.println("test11111111111");
//            frequency fre_correlation =new frequency(new Path().getYuanPathContain()+words[i]+".txt",new Path().getYuanPathCorrelation()+words[i]+".txt");
//
//            TransFormToTxt transFormToTxt=new TransFormToTxt(readData_contain,i+1+"",fre_correlation);
//
...
//
//        for(int i=0;i<words.length;i++){
//            ReadMediation readMediation= new ReadMediation(new Path().getYuanPathCorrelation()+words[i]+".txt");
...
//////            if(i==words.length-1) {
//////                for (int y = 0; y < mediation.length; y++) {
//////                    System.out.println(new String(mediation[y].getBytes("gbk"), StandardCharsets.UTF_8));
//////                }
//////            }
//            long sCount= new ReadLineCount(new Path().getYuanPathContain() + words[i] + ".txt").readLineCount();
//            int[] sa = new ReadSa(new Path().getYuanPathContain() + words[i] + ".txt",words[i],mediation).readSa();
//            float[] w = new float[new ReadMediation().getNum()];
//            for(int j=0;j<w.length;j++){
//                w[j]=((float)sa[j])/sCount;
//                //System.out.println(w[j]);
//            }
//            ReadContainMediation readContainMediation = new ReadContainMediation(new Path().getYuanPathClean(),new Path().getYuanPathContain() +"Mediation"+ words[i] + ".txt",mediation);
//            int[] a=readContainMediation.readContainMediation();
//            frequency frequency = new frequency(new Path().getYuanPathContain() +"Mediation"+ words[i] + ".txt",new Path().getYuanPathSelectKey()+ words[i] + ".txt");
//            frequency.getFrequency();
//            ReadCompete readCompete = new ReadCompete(new Path().getYuanPathSelectKey()+ words[i] + ".txt",new Path().getYuanPathKey()+ words[i] + ".txt",mediation);
//            String[] keyWord=readCompete.readCompete();
//            ReadKa readKa=new ReadKa(new Path().getYuanPathContain() +"Mediation"+ words[i] + ".txt",keyWord,mediation);
...
////                System.out.println(" ");
////            }
//            computeComp computeComp = new computeComp(new Path().getYuanPathComp()+ words[i] + ".txt",w,kA,a,sa,keyWord);
...
//    }
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        try {
            ...
            document = saxReader.read(new File("D:\\软件度量\\web1\\Electronic-Commerce\\src\\main\\java\\com\\csu\\ecbackend\\test1.xml")); // 读取XML文件,获得document对象
